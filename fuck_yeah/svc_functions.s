...
; returns True if the button in R0 is currently pressed is_button_pressed
	PUSH	{R1}
	MOV	R1, R0
	SVC	SVC_PollButts
	TST	R1, R0
...
setcur	PUSH	{R2}
	...
	; if col < 0; col = CURSOR_COLS; row -= 1
	CMP	R0, #0
	MOVLT	R0, #CURSOR_COLS
	SUBLT	R1, R1, #1
	BLT	wcur
...
; returns the current state of the button registered
poll_buttons
	LDR	R0, =keypad_ABCD
	LDR	R0, [R0]
	MOV	PC, LR

; read into R0 from the interrupt bits port
read_interrupt_bits
	LDR	R0, =INT_MASK_REGISTER
	LDRB	R0, [R0]
	MOV	PC, LR

; store R0 to interrupt bits port
store_interrupt_bits
	PUSH	{R1}
	LDR	R1, =INT_MASK_REGISTER
	STRB	R0, [R1]
	POP	{R1}
	MOV	PC, LR

; read into R0 from the timer compare register
read_timer_compare
	LDR	R0, =IO_TIMER_COMPARE
	LDR	R0, [R0]
	MOV	PC, LR

; store R0 to the timer compare register
store_timer_compare
	PUSH	{R1}
...
; read into R0 from the control register
read_control_register
	LDR	R0, =IO_CONTROL_REGISTER
	LDR	R0, [R0]
	MOV	PC, LR

; store R0 to the control register
store_control_register
	PUSH	{R1}
	LDR	R1, =IO_CONTROL_REGISTER
	STR	R0, [R1]
	POP	{R1}
	MOV	PC, LR

; read into R0 from the keyboard matrix
read_keyboard_matrix
	LDR	R0, =KEYBOARD_MATRIX
	LDRB	R0, [R0]
	MOV	PC, LR

; store R0 to the keyboard matrix
store_keyboard_matrix
	PUSH	{R1}
	LDR	R1, =KEYBOARD_MATRIX
	STRB	R0, [R1]
	POP	{R1}
	MOV	PC, LR

; read into R0 from the keyboard matrix direction register
read_keyboard_matrix_direction
	LDR	R0, =KEYBOARD_MATRIX_DIRECTION
	STRB	R0, [R0]
	MOV	PC, LR

; store R0 to the keyboard matrix direction register
store_keyboard_matrix_direction
	PUSH	{R1}
	LDR	R1, =KEYBOARD_MATRIX_DIRECTION
	STRB	R0, [R1]
	POP	{R1}
	MOV	PC, LR
